...
// Cgroup connect6 program for process isolation
...
// Process policy structure for IPv6
struct process_policy_v6 {
    __u32 pid;
    __u32 uid;
    __u32 gid;
    __u32 src_ip[4];  // IPv6 address as 4 u32s
    __u32 dst_ip[4];  // IPv6 address as 4 u32s
    __u16 src_port;
    __u16 dst_port;
    __u8 protocol;
...
struct {
    ...
    __type(key, __u32);
    __type(value, struct process_policy_v6);
} process_policies_v6 SEC(".maps");
...
// Helper function to check IPv6 address match
static __always_inline int ipv6_match(__u32 *ip, __u32 *target) {
    return ip[0] == target[0] && ip[1] == target[1] && 
           ip[2] == target[2] && ip[3] == target[3];
}
...
int cgroup_connect6_prog(struct bpf_sock_addr *ctx) {
    ...
    __u64 *total = bpf_map_lookup_elem(&connect_stats_v6, &key);
    if (total) {
    ...
    // Extract IPv6 addresses (simplified)
    // In real implementation, you'd extract from ctx->user_ip6
    src_ip[0] = 0; src_ip[1] = 0; src_ip[2] = 0; src_ip[3] = 0;
    dst_ip[0] = 0; dst_ip[1] = 0; dst_ip[2] = 0; dst_ip[3] = 0;
    
    // Check policies
    struct process_policy_v6 *policy;
    __u32 policy_key = 0;
    
    // Simple policy lookup (in real implementation, use more sophisticated matching)
    policy = bpf_map_lookup_elem(&process_policies_v6, &policy_key);
    if (policy) {
        // Check if connection matches policy
        if (ipv6_match(src_ip, policy->src_ip) &&
            ipv6_match(dst_ip, policy->dst_ip) &&
            ...
            if (policy->action == 1) { // ALLOW
                ...
                __u64 *allowed = bpf_map_lookup_elem(&connect_stats_v6, &key);
                if (allowed) {
                ...
                __u64 *logged = bpf_map_lookup_elem(&connect_stats_v6, &key);
                if (logged) {
    ...
    __u64 *blocked = bpf_map_lookup_elem(&connect_stats_v6, &key);
    if (blocked) {
