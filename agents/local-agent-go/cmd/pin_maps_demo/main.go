...
func main() {
	...
	fmt.Println("=== AegisFlux Policy Maps Pinning and Seeding Demo ===")
	...
	// Generate bpftool commands for verification
	fmt.Println("\n=== bpftool Verification Commands ===")
	fmt.Println("On Linux, you can verify the maps with these commands:")
	fmt.Println("  sudo bpftool map dump pinned /sys/fs/bpf/aegis/policy_edges")
	fmt.Println("  sudo bpftool map dump pinned /sys/fs/bpf/aegis/allow_lpm4")
	fmt.Println("  sudo bpftool map show pinned /sys/fs/bpf/aegis/policy_edges")
	fmt.Println("  sudo bpftool map show pinned /sys/fs/bpf/aegis/allow_lpm4")
	...
	if runtime.GOOS == "linux" {
		fmt.Println("\n🚀 This would work with real eBPF maps on Linux!")
	} else {
		fmt.Println("\n📝 This is a mock implementation for macOS testing.")
